...
$doc->title = __('Переписка с "%s"', $ank->nick);
if(isset($_GET['delete']) && is_numeric($_GET ['delete'])){
$id_mail = (int) $_GET ['delete'];
$q = $db->prepare("SELECT * FROM `mail` WHERE (`id_user` = ? AND `id_sender` = ?) OR (`id_user` = ? AND `id_sender` = ?) && `id` = ? LIMIT 1");
$q->execute(Array($user->id,$id_kont,$id_kont,$user->id,$id_mail));
if (!$mail_message= $q->fetch()) {
...
if($mail_message['is_read'] == 0){
if (isset($_GET ['return']))
header('Refresh: 1; url=' . $_GET ['return']);
else
header('Refresh: 1; url=./');
$doc->err(__('Сообщение еще не прочитано'));
exit();
}
$res = $db->prepare("UPDATE `mail` SET `mess` = ? WHERE `id` = ? LIMIT 1");
$res->execute(Array(NULL, $id_mail));
...
$post->image = $ank2->ava();
if($mail['mess']){$post->content = text::toOutput($mail ['mess']);} else {$post->content = '<font color="red">[Сообщение удалено] </font>';}
$post->highlight = !$mail ['is_read'];
$post->time = misc::when($mail ['time']);
if($user->id == $ank2->id && ($mail['mess'])) { 
if($mail['is_read'] == 1)$post->action('delete', '?id=' . $id_kont . '&amp;delete=' . $mail['id'] . '&amp;return=' . URL);
}
