...
if (!empty($_FILES ['file'])) {
    if ($_FILES ['file'] ['error']) {
        ...
        if ($files_ok = $avatars_dir->filesAdd(array($_FILES ['file'] ['tmp_name'] => $avatar_file_name))) {
            ...
            unset($files_ok);
$q = $db->prepare("DELETE FROM `ava_komm` WHERE `id_ava` = ?");
$q->execute(Array($user->id));
$q = $db->prepare("DELETE FROM `ava_like` WHERE `id_ava` = ?");
$q->execute(Array($user->id));
 $user->avatar = 1;
...
// Аватар 
if ($avatars_dir->is_file($avatar_file_name)) { 
$path = $user->getAvatar($doc->img_max_width());
...
if (!empty($_POST['delete'])) {
 ...
 $user->avatar = 0;
    ...
    echo "<img class='photo' src='" . $path . "' alt='".__('Мой аватар')."' style='max-height:300px;' /><br />\n";
