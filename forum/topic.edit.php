...
$doc = new document (2);
if(!$user->access('forum_edit_razdel'))$doc->access_denied(__('У Вас нет доступа!'));
...
$q = $db->prepare("SELECT * FROM `forum_topics` WHERE `id` = ?");
$q->execute(Array($id_topic));
if (!$topic = $q->fetch()) {
    header('Refresh: 1; url=./');
    $doc->err(__('Раздел не существует'));
    exit;
}
if (isset($_POST ['save'])) {
    if (isset($_POST ['name'])) {
...
if($user->access('forum_theme_delete'))$doc->act(__('Удаление тем'), 'topic.themes.delete.php?id=' . $topic ['id']);
if($user->access('forum_del_razdel'))$doc->act(__('Удалить раздел'), 'topic.delete.php?id=' . $topic ['id']);
