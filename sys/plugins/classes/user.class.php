...
function ava($html = true)
{
$avatar_file_name = $this->id . '.jpg';
$avatars_path = FILES.'/.avatars'; // папка с аватарами
$avatars = FILES.'/.avatars/'.$this->id . '.jpg'; 
$avatars_dir = new files($avatars_path);
if ($avatars_dir->is_file($avatar_file_name)) {
$avatar = new avatar($avatars_path, $avatar_file_name);
return $avatar->getScreen(450, 0);
}
else {
$file = new avatar($avatars_path, 'standart.jpg');
return $file->getScreen(150, 0);
}
}
function access($access, $group = false)
{
global $dcms ;
$group = 'access_' . ($group !== false ? $group : $this->group) ;
$access = strripos($dcms->$group, ':' . $access . ':') ;
...
if (!$html)
return $login;
$ret = array('<img src="/sys/images/icons/'.$this->icon().'.png"/> <span class="' . ($this->online ? 'DCMS_nick_on' : 'DCMS_nick_off') . '">' . $login . '</span>');
if ($this->donate_rub)
...
$file = new files_file($avatars_path, 'standart.jpg');
return $file->getScreen($max_width, 0);
}
}
