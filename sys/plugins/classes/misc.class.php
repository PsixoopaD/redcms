...
static function vremja_sec($time){ 

$year = floor($time / (3600 * 24 * 7 * 4 * 12)) ; 
$time = $time - (3600 * 24 * 7 * 4 * 12 * $year) ; 

$mounth = floor($time / (3600 * 24 * 7 * 4)) ; 
$time = $time - (3600 * 24 * 7 * 4 * $mounth) ; 

$week = floor($time / (3600 * 24 * 7)) ; 
$time = $time - (3600 * 24 * 7 * $week) ; 
...
$year = ($year > 0 ? $year . misc::number($year, ' год ', ' года ', ' лет ') : NULL) ; 
$mounth = ($mounth > 0 ? $mounth . misc::number($mounth, ' месяц ', ' месяца ', ' месяцев ') : NULL) ; 
$week = ($week > 0 ? $week . misc::number($week, ' неделя ', ' недели ', ' недель ') : NULL) ; 
$day = ($day > 0 ? $day . misc::number($day, ' день ', ' дня ', ' дней ') : NULL) ; 
$hour = (!empty($hour)?$hour . misc::number($hour, ' час ', ' часа ', ' часов ') : NULL) ; 
$minute = (!empty($minute)?$minute . misc::number($minute, ' минута ', ' минуты ', ' минут ') : NULL) ; 
$second = (!empty($second)?$second . ' сек.' : NULL) ; 

return $year . $mounth . $week . $day . $hour . $minute . $second ; 
}
