...
if(!$user->access('set_group_user'))$doc->access_denied(__('У Вас нет доступа!'));
$doc = new document(2);
    ...
    if ($group_now >= $user->group)
        ...
        if ($group_last != $group_now) {
            ...
            $dcms->log('Пользователи',
                'Изменение группы пользователя [url=/profile.view.php?id=' . $ank->id . ']' . $ank->nick. '[/url] с ' . groups::name($group_last) . ' на ' . groups::name($ank->group));

            $doc->msg(__('Пользователь "%s" теперь "%s"', $ank->nick, groups::name($ank->group)));
        }
    }
}
